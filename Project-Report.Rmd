...
dat %>%
  ...
  summarize(ave_delay = mean(dep_delay)) %>% 
...
<aside>
Exercise: What to do? (Best quess)
</aside>
...
dat1 %>%
  ...
  geom_col() +
  labs(
    title = "Number of flights to each destination",
    x = "Number of fligths",
    y = "Destination"
  ) +
  theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1))
...
dat_o <- flights %>% 
  ...
  left_join(airports %>% select(faa, name), 
            by = c("dest" = "faa")) %>% 
  rename(dest_name = name) %>% 
  select(month, carrier_name, origin_name, dest_name, dep_time, dep_delay, arr_delay, distance, )
