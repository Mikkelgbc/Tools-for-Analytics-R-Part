...
#   ggplot(aes(origin_name, ave_delay)) +
#   geom_boxplot() +
#   coord_cartesian(ylim = c(0,0.5)) +
#   labs(
#     title = "Average departure delays",
...
dat %>%
  ...
  summarise(ave_delay = mean(dep_delay)) %>% 
  ggplot() +
  stat_ecdf(aes(ave_delay, color = origin_name), alpha = 0.75) +
  coord_cartesian(xlim = c(0,1)) +
  facet_wrap(vars(carrier_name)) +
  labs(
    title = "Distribution of average departure delays by airport",
    x = "Delay (hours)",
    y = "Density",
    color = "Carrier"
  ) +
...
```{r}
dat %>% 
  ggplot(aes(sched_dep_time, arr_delay)) +
  geom_point() +
