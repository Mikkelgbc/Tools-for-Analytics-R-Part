...
dat %>% 
  ...
  geom_boxplot() +
  labs(
    title = "Boxplot of departure delays by airline",
    x = "Carrier",
    y = "Delay (hours)"
    ) +
  theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1))
...
It is evident that John F Kennedy Intl have the largest and most common outliers, while La Guardia and Newark Liberty Intl has fewer outliers and a lower standard deviation
...
dat %>% 
  ...
  geom_col() +
  labs(
    title = "Number of delayed flighs pr. airline",
    x = "Carrier",
    y = "Fligths delayed more than 1 hour"
  ) +
  theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1))
```
...
We see that there are a linear relation between Departure Delay and Arrival Delay, with most flights being delay between 0 and 5 hours, and then som huge outliers of between 10 and 22 hours
...
<aside>
Exercise: What to do? (Best guess)
</aside>
...
dat %>%
  ...
  summarize(avg_delay = mean(dep_delay)) %>% 
  ggplot() +
  stat_ecdf(aes(avg_delay, color = origin_name), alpha = 0.75) +
  coord_cartesian(xlim = c(0,1)) +
  facet_wrap(vars(carrier_name)) +
  labs(
    title = "Distribution of departure delays for carriers by airport",
...
Both in terms of departure and arrival, there seems to be more delay on shorter fligths from 0 to 1100 km. Departure delay seems to flatline afterwards, while arrival delay continues to decrease as the flight distance increases.
...
dat1 %>%
  group_by(dest_name) %>% 
  count(dest_name) %>% 
  ggplot(aes(n, reorder(dest_name, n))) +
  geom_col() +
  labs(
...
dat_w %>% 
  ...
  labs(
    title = "Fluctutating temperature over the year",
    x = "Time",
    y = "Temperature",
    color = "Airport"
  )
...
Note that we have used the `alpha` aesthetic to make the lines more transparent. We see that all three airport follow the same trend in temperature. However, John F Kennedy Intl does have a sligtly lower temparture from april to october compared the others.
...
dat_w %>% 
  ...
  geom_density() +
  labs(
    title = "Are the temparatures different in the airports?",
    x = "Temperature",
    y = "Density",
    color = "Airport"
  )
...
dat_w %>% 
  ggplot() +
  stat_ecdf(aes(temp, color = origin_name), alpha = 0.75) +
  labs(
    title = "Are the temparatures different in the airports?",
    x = "Temperature",
    y = "Density",
...
The dot in the boxplot of may is an outlier of -10 degress in a month where the average temperature is around 20 degress. This could be explained by a sensor failure, resulting in a false observation.
...
<aside>
Exercise: Add a few plots you think are important.
</aside>
...
<aside>
Exercise: Include further analysis you think are important.
</aside>
...
```{r}
dat1 <- dat
dat1 [which(dat1 $plane_manufacturer == "AIRBUS INDUSTRIE"),] $plane_manufacturer <- "AIRBUS"
dat1 [which(dat1 $plane_manufacturer == "CANADAIR LTD"),] $plane_manufacturer <- "CANADAIR"
dat1 [which(dat1 $plane_manufacturer %in% c("MCDONNELL DOUGLAS AIRCRAFT CO", "MCDONNELL DOUGLAS CORPORATION")),] $plane_manufacturer <- "MCDONNELL DOUGLAS"
```
