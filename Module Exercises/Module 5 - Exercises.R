...
greedy <- function(startup, setup) {
  jobs <- nrow(setup) # Defines the number of jobs
  cur_job <- which.min(startup) # Defines current job as the job with the lowest startup cost
  cost <- startup[cur_job] # Cost is equal to the startup cost of the current job
  
  # cat("Start job:", cur_job, "\n")
  job_seq <- cur_job # Sets job sequence equal to the current job
  setup[, cur_job] <- NA # We set the columns of the current jobs startup costs to NA to exclude it from being picked again.
  
  for (i in 1:(jobs-1)) { # Loops trough the remaining jobs and set the next job equal to job with the lowest startup cost.
    next_job <- which.min(setup[cur_job, ])
    
    # cat("Next job:", next_job, "\n")
    cost <- cost + setup[cur_job, next_job] # Updates costs during loop
    job_seq <- c(job_seq, next_job) # Notes the job sequence
    cur_job <- next_job # Sets the current job equal to the next job
    setup[, cur_job] <- NA # Set the columns of the current jobs startup costs to NA to exclude it from being picked again.
  }
  # print(setup)
  return(list(seq = job_seq, cost = cost)) # Returns a list of the job sequence and cost
}

greedy(startup_costs, setup_costs) # Returns the result of the function
...
# Task: Implement a better function calculating a better strategy. 
# Hint: to find the minimum column costs, you may use apply(rbind(startup, setup), 2, min, na.rm = T).
better <- function(startup,setup){
  jobs <- nrow(setup) # Defines the number of jobs
  min_col_val <- apply(rbind (startup, setup), 2, min, na.rm = TRUE) # Returns a new vector with the minimum column values
  startup <- startup - min_col_val # Subtracts the minimum column value from the startup costs matrix
  min_mat <- matrix(rep(min_col_val, jobs), ncol = jobs, byrow = TRUE) # Creates a matrix of the minimum values by size jobs x jobs (5 x 5)
  setup <- setup - min_mat # Subtracts the minimum matrix from the setup costs
  lst <- greedy(startup, setup) # Call the greedy function with the modified setup costs
  lst$cost <- lst$cost + sum(min_col_val) # We add the values back to the costs
